...
$pesanan = $id ? Pesanan::find($id) : null;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        // Mengecek apakah pesanan ini diproses Gudang langsung (pesanan_id) ATAU via SPK (spk_id)
        $stmtCekGudang = $pdo->prepare("
            SELECT COUNT(id) 
            FROM pengeluaran 
            WHERE pesanan_id = ? OR spk_id IN (SELECT id FROM spk WHERE pesanan_id = ?)
        ");
        $stmtCekGudang->execute([$id, $id]);
          ...
          <div style="background: white; padding: 1.25rem; border-radius: 0.5rem; margin-bottom: 1.5rem; border: 1px solid #ffe0e0;">
            ...
            <div class="detail-grid">
              <div class="detail-item">
                ...
                <span class="detail-val fw-mid" style="color: #dc3545; font-size: 1.1rem;"><?= htmlspecialchars($pesanan['nomor_pesanan'] ?? '-') ?></span>
              </div>
              <div class="detail-item">
                <span class="detail-label">Customer / Perusahaan</span>
                <span class="detail-val"><?= htmlspecialchars($pesanan['perusahaan'] ?? $pesanan['customer'] ?? '-') ?></span>
              </div>
              <div class="detail-item">
              ...
              <div class="detail-item">
                ...
                <span class="detail-val"><?= htmlspecialchars($pesanan['status'] ?? '-') ?></span>
              </div>
            </div>
          </div>
      ...
      <div class="form-side">
        <div class="form-card" style="border-left: 4px solid #dc3545;">
          ...
          <ul class="info-list" style="font-size: 0.9rem;">
            ...
            <li style="margin-bottom: 0.75rem;"><i class="bi bi-check2-square"></i> <strong>Verifikasi</strong> tidak ada items/barang dalam pesanan</li>
