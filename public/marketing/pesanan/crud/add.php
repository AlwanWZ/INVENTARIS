...
try {
    ...
    $stmtOrder = $pdo->prepare("SELECT nomor_pesanan FROM pesanan WHERE nomor_pesanan LIKE ? ORDER BY id DESC LIMIT 1");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$dataPO['nomor_pesanan']) $errors[] = 'Nomor Pesanan wajib diisi.';
...
if (!empty($dataPO['tanggal_pengiriman']) && $dataPO['tanggal_pengiriman'] < $dataPO['tanggal']) {
    ...
    if (isset($_POST['items']) && is_array($_POST['items'])) {
        foreach ($_POST['items'] as $idx => $item) {
            ...
            // FIX LOGIKA: Cari data barang langsung dari $produkList yang udah ditarik di awal, 
            // jangan pakai Barang::find() biar pasti dapat kolom stoknya.
            $barang = null;
            if (!empty($item['barang_id'])) {
                foreach ($produkList as $p) {
                    if ($p['id'] == $item['barang_id']) {
                        $barang = $p;
                        break;
                    }
                }
            ...
            // STRICT VALIDATION DIHAPUS: 
            // Sekarang diperbolehkan order melebihi stok, stok_available bisa minus (Backorder)
            // SPK nanti akan menutupi kekurangan ini.

            $dataItems[] = [
                'barang_id'     => intval($item['barang_id'] ?? 0) ?: null,
                'kode_material' => $barang['kode_barang'] ?? trim($item['kode_material'] ?? ''),
                'nama_material' => $barang['nama'] ?? trim($item['nama_material'] ?? ''),
    ...
    if (!$errors) {
        try {
            ...
            $poId = Pesanan::createWithItems($dataPO, $dataItems);
...
<div class="form-grid-3">
  <div class="form-group">
    ...
    <input type="text" name="nomor_pesanan" class="form-control"
           value="<?= htmlspecialchars(($_POST['nomor_pesanan'] ?? $autoNomorPo) ?: '') ?>"
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    function createRow() {
      ...
      tr.innerHTML = `
        ...
        <td>
          <div>
            <select name="items[${currentRowCount}][barang_id]" class="form-control select-barang" required>
              <option value="">-- Pilih Produk --</option>
              ${produkList.map(p => `<option value="${p.id}" data-harga="${p.harga || 0}" data-satuan="${p.satuan || 'pcs'}" data-kode="${p.kode_barang || p.kode || ''}" data-nama="${p.nama || ''}" data-stok="${p.stok || 0}" data-stok-available="${p.stok_available || p.stok || 0}">${p.nama} (${p.kode_barang || p.kode || ''})</option>`).join('')}
            </select>
      ...
      // 1. Saat Pilih Produk
      const selectProduk = tr.querySelector('.select-barang');
      ...
      qtyInput.addEventListener('input', function() {
        ...
        // VALIDASI LONGGAR (BISA BACKORDER / BUAT SPK)
        if (qty > stok && qty > 0) {
          // INFO - QTY LEBIH DARI STOK (BUTUH SPK)
          const kurang = qty - stok;
          qtyWarning.innerHTML = `<span style="color: #f59e0b; font-weight: 600;">⚠️ Stok kurang ${kurang} pcs (SPK akan dibuat)</span>`;
          qtyWarning.style.display = 'block';
          this.style.borderColor = '#f59e0b';
          this.style.backgroundColor = 'rgba(245, 158, 11, 0.05)';
        } else if (qty > 0 && qty <= stok) {
          // OK - HIJAU
          qtyWarning.innerHTML = `<span style="color: #28a745; font-weight: 600;">✅ Stok Aman (${stok - qty} pcs tersisa)</span>`;
          qtyWarning.style.display = 'block';
    ...
    function validateAllRows() {
      ...
      document.querySelectorAll('.item-row').forEach(row => {
        ...
        // Cek: qty valid (hanya cek tidak boleh <= 0 atau kosong, stok tidak dibatasi)
        if (qty <= 0) {
          isValid = false;
          return;
        }
