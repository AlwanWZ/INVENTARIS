...
// Calculate the total dynamically from pesanan_items
$totalAmount = Pesanan::calculateTotal($pesanan['id']);

$statusCls = match($pesanan['status']) {
    'approved' => 'ok',
    'proses'    => 'warn',
    'rejected' => 'danger',
    default     => 'neutral'
  ...
  <div class="content">
    ...
    <div class="topbar">
      <div class="top-left">
        ...
        <div class="breadcrumb">
          ...
          <i class="bi bi-chevron-right"></i>
          <span><?= htmlspecialchars($pesanan['nomor_pesanan']) ?></span>
        </div>
      </div>
      <div class="top-right">
    ...
    <div class="page-header">
      <div class="page-header-left">
        ...
        <p class="page-subtitle">Order <strong><?= htmlspecialchars($pesanan['nomor_pesanan']) ?></strong> dari <strong><?= htmlspecialchars($pesanan['perusahaan'] ?? '-') ?></strong></p>
      </div>
      ...
      <div class="detail-main">
        <div class="form-card">
          <div class="form-card-header">
            ...
            <span class="badge <?= $statusCls ?>"><?= htmlspecialchars($pesanan['status']) ?></span>
          </div>
          <div class="detail-grid">
            ...
            <div class="detail-item">
              ...
              <span class="detail-val"><?= htmlspecialchars($pesanan['customer'] ?? $pesanan['perusahaan'] ?? '—') ?></span>
            </div>
            <div class="detail-item">
        ...
        <div class="form-card">
          <div class="form-card-header" style="display: flex; justify-content: space-between; align-items: center;">
            ...
            <a href="add_item.php?pesanan_id=<?= $pesanan['id'] ?>" class="btn-primary" style="padding: 0.5rem 1rem; font-size: 0.9rem;">
                ...
                <?php if (empty($poItems)): ?>
                  <tr>
                    <td colspan="9" class="empty-state">
                      <i class="bi bi-box"></i>
                      <span>Belum ada item dalam order ini. <a href="add_item.php?pesanan_id=<?= $pesanan['id'] ?>" style="color: #007bff; font-weight: 500;">Tambah Item</a></span>
                    </td>
                  </tr>
                <?php else: ?>
        ...
        <div class="form-card">
          ...
          <div class="action-body">
            <?php if ($pesanan['status'] === 'draft'): ?>
              ...
              <!-- BUTTON 2: REJECT -->
              <a href="edit.php?id=<?= $pesanan['id'] ?>&action=reject" class="btn-danger full" onclick="return confirm('Reject order ini?')">
          ...
          <div style="padding: 1rem; font-size: 0.9rem;">
            <div style="margin-bottom: 0.8rem;">
              ...
              <span class="badge" style="margin-left: 0.5rem;">
                <?php 
                  $stok_status = $pesanan['status_stok'] ?? 'draft';
            ...
            <div style="padding: 0.8rem; background: #f0f7ff; border-left: 3px solid #0d6efd; border-radius: 4px; color: #0c5da7; font-size: 0.85rem; line-height: 1.5;">
              ...
              • Stok barang akan berkurang sesuai qty ready<br>
...
<script>
const poId = <?= json_encode($pesanan['id']) ?>;
const poStatus = <?= json_encode($pesanan['status']) ?>;
const poNumber = <?= json_encode($pesanan['nomor_pesanan']) ?>;

async function approveAndReserve() {
  if (!confirm('Approve order ini dan reserve stok?\n\nStok barang akan berkurang sesuai qty ready yang dipesan.')) {
    return;
  }
  ...
  try {
    const response = await fetch('reserve_stok.php', {
      ...
      body: `pesanan_id=${poId}&action=reserve`
    });
...
async function unreserveStok() {
  ...
  try {
    const response = await fetch('reserve_stok.php', {
      method: 'POST',
      headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
      body: `pesanan_id=${poId}&action=unreserve`
    });
