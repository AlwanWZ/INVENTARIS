...
$pesanan = Pesanan::find($_GET['pesanan_id'] ?? null);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Get barang data for auto-fill
    $barang = $_POST['barang_id'] ? Barang::find($_POST['barang_id']) : null;
    
    $data = [
        'pesanan_id'         => $pesanan['id'],
        'barang_id'     => intval($_POST['barang_id'] ?? 0),
        'kode_material' => $barang['kode_barang'] ?? trim($_POST['kode_material'] ?? ''),
        'nama_material' => $barang['nama'] ?? trim($_POST['nama_material'] ?? ''),
        'uom'           => $_POST['uom'] ?? 'pcs',
        'qty'           => intval($_POST['qty'] ?? 0),
        'harga_satuan'  => intval(str_replace(['.', ','], '', $_POST['harga_satuan'] ?? $barang['harga'] ?? 0)),
    ...
    if ($barang) {
        $stok_available = $barang['stok_available'] ?? $barang['stok'] ?? 0;
        
        // Jika qty > stok, REJECT!
        if ($data['qty'] > $stok_available) {
            $errors[] = "Qty ({$data['qty']}) melebihi stok '{$barang['nama']}' ({$stok_available} pcs). Maksimal: {$stok_available} pcs";
        } else {
    ...
    if (!$errors) {
        Pesanan::addItem($data);
        header('Location: detail.php?id=' . $pesanan['id'] . '&item_added=1');
        exit;
    }
}
?>
  ...
  <div class="content">
    ...
    <div class="page-header">
      <div class="page-header-left">
        ...
        <p class="page-subtitle">Tambahkan barang/PCB ke pesanan <strong><?= htmlspecialchars($pesanan['nomor_pesanan']) ?></strong> dari <strong><?= htmlspecialchars($pesanan['perusahaan'] ?? '-') ?></strong></p>
      </div>
    ...
    <div class="form-layout">
      <div class="form-main">
        <div class="form-card">
          ...
          <form method="post" class="pesanan-form" id="addItemForm">
            <div class="form-group">
              <label class="form-label">Pilih Produk PCB <span class="required">*</span></label>
              <select name="barang_id" class="form-control" id="produkSelect" onchange="autofillProduk()">
                <option value="">-- Pilih Produk --</option>
                <?php foreach ($produkList as $prod): ?>
                  <option value="<?= $prod['id'] ?>" 
                          data-kode="<?= htmlspecialchars($prod['kode_barang']) ?>" 
                    ...
                    <?= htmlspecialchars($prod['kode_barang'] . ' - ' . $prod['nama']) ?>
