...
$stmtPo = $pdo->prepare("
    SELECT pesanan.*, c.nama AS customer_nama, c.perusahaan AS customer_pt, c.alamat AS customer_alamat, c.no_hp AS customer_no_hp 
    FROM pesanan 
    LEFT JOIN customers c ON pesanan.customer_id = c.id 
    WHERE pesanan.id = ?
");
$stmtPo->execute([$poId]);
$pesanan = $stmtPo->fetch(PDO::FETCH_ASSOC);
...
$tanggal_pesanan   = $pesanan['tanggal'] ?? date('Y-m-d');
$creator           = $pesanan['created_by'] ?? $_SESSION['user']['username'] ?? '-';
$customer_nama     = $pesanan['customer_nama'] ?? $pesanan['customer'] ?? 'Nama Customer';
$customer_pt       = $pesanan['customer_pt'] ?? 'Nama Perusahaan'; // Data baru perusahaan
$alamat_customer   = $pesanan['customer_alamat'] ?? 'Alamat tidak ditemukan'; 
$telepon_customer  = $pesanan['customer_no_hp'] ?? '-'; 
$catatan_po        = $pesanan['notes'] ?? ''; 
