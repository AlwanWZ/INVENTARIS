...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['nuke_id'])) {
    ...
    try {
        ...
        foreach ($tabel_anak as $tabel) {
            try {
                $pdo->prepare("DELETE FROM {$tabel} WHERE barang_id = ?")->execute([$nukeId]);
            } catch (Exception $e) {
...
<div class="container">
    ...
    <p>Halaman ini akan memaksa database menghapus barang dan menyapu bersih histori di <strong>semua tabel</strong>.</p>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                <td><?= $p['id'] ?></td>
                <td><?= htmlspecialchars($p['kode'] ?? $p['kode_barang'] ?? '') ?></td>
                <td><strong><?= htmlspecialchars($p['nama'] ?? '') ?></strong></td>
                <td><?= htmlspecialchars($p['status'] ?? 'null') ?></td>
                <td><?= $p['stok'] ?? 0 ?></td>
                <td>
                    <form method="post" onsubmit="return confirm('Bantai barang ini tanpa ampun dari SEMUA tabel?');">
