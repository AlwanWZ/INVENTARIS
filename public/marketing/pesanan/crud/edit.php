...
if (isset($_GET['action'])) {
    ...
    if ($action === 'approve') {
        ...
        Pesanan::update($pesanan['id'], $data);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$data['nomor_pesanan']) $errors[] = 'Nomor Pesanan wajib diisi.';
    ...
    // Merge POST ke $pesanan agar form tetap tampil nilai baru saat ada error
    $pesanan = array_merge($pesanan, $data);
}
$statusCls = match($pesanan['status']) { 'Disetujui' => 'ok', 'Proses' => 'warn', default => 'neutral' };
?>
<!doctype html>
    ...
    <div class="topbar">
      <div class="top-left">
        ...
        <div class="breadcrumb">
          ...
          <i class="bi bi-chevron-right"></i>
          <a href="detail.php?id=<?= $pesanan['id'] ?>"><?= htmlspecialchars($pesanan['nomor_pesanan']) ?></a>
          <i class="bi bi-chevron-right"></i>
          <span>Edit</span>
        </div>
      </div>
    ...
    <div class="page-header">
      <div class="page-header-left">
        ...
        <p class="page-subtitle">
          Order <strong><?= htmlspecialchars($pesanan['nomor_pesanan']) ?></strong>
          dari <strong><?= htmlspecialchars($pesanan['perusahaan'] ?? '-') ?></strong>
          &mdash; <span class="badge <?= $statusCls ?>"><?= htmlspecialchars($pesanan['status']) ?></span>
        </p>
      </div>
      <div class="header-actions">
        <a href="detail.php?id=<?= $pesanan['id'] ?>" class="btn-ghost-sm">
      ...
      <div class="form-main">
        <div class="form-card">
          <div class="form-card-header">
            ...
            <span class="badge <?= match($pesanan['status']) { 'approved' => 'ok', 'rejected' => 'danger', default => 'neutral' } ?>"><?= htmlspecialchars($pesanan['status']) ?></span>
          </div>
          ...
          <form method="post" class="pesanan-form" id="editForm">
            <div class="form-row">
            ...
            <div class="form-group">
              ...
              <select name="customer_id" class="form-control" required>
                ...
                <?php foreach ($customerList as $cust): ?>
                  <option value="<?= $cust['id'] ?>" <?= intval($pesanan['customer_id'] ?? 0) === $cust['id'] ? 'selected' : '' ?>>
            ...
            <div class="form-row">
              <div class="form-group">
                ...
                <select name="status" class="form-control">
                  <option value="draft" <?= $pesanan['status'] === 'draft' ? 'selected' : '' ?>>Draft (Baru)</option>
                  <option value="pending_review" <?= $pesanan['status'] === 'pending_review' ? 'selected' : '' ?>>Pending Review (Tunggu Manager)</option>
                  <option value="approved" <?= $pesanan['status'] === 'approved' ? 'selected' : '' ?>>Approved (Disetujui)</option>
                  <option value="rejected" <?= $pesanan['status'] === 'rejected' ? 'selected' : '' ?>>Rejected (Ditolak)</option>
                </select>
            ...
            <div class="form-actions">
              ...
              <a href="detail.php?id=<?= $pesanan['id'] ?>" class="btn-outline">Batal</a>
            </div>
          </form>
        </div>
      </div>
        ...
        <div class="form-card">
          ...
          <div class="side-info-list">
            ...
            <div class="side-info-item">
              ...
              <span class="side-info-val"><span class="badge <?= $statusCls ?>"><?= htmlspecialchars($pesanan['status']) ?></span></span>
            </div>
          </div>
        </div>
