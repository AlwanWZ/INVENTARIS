...
$list      = $pengeluaranModel->getAll($search, $status);
...
<head>
  ...
  <title>Pengeluaran Barang | Inventory</title>
...
<main class="main">
  <div class="content">
    ...
    <?php if (isset($_GET['success'])): ?><div class="alert-success"><i class="bi bi-check-circle"></i> Pengeluaran berhasil disimpan.</div>
    <?php elseif (isset($_GET['updated'])): ?><div class="alert-success"><i class="bi bi-check-circle"></i> Pengeluaran berhasil diperbarui.</div>
    <?php elseif (isset($_GET['deleted'])): ?><div class="alert-warn"><i class="bi bi-trash"></i> Pengeluaran berhasil dihapus.</div>
    <?php endif; ?>

    <div class="page-header">
      <div class="page-header-left">
        <h1 class="page-title-lg">Pengeluaran Barang</h1>
        <p class="page-subtitle">Kelola pengeluaran barang dari gudang ke divisi atau tujuan.</p>
      </div>
      <a href="crud/add.php" class="btn-primary"><i class="bi bi-plus-lg"></i> Tambah Pengeluaran</a>
    </div>
    ...
    <div class="form-card filter-card">
      ...
      <form method="get" class="filter-form">
        <div class="filter-group filter-search">
          ...
          <input type="text" name="search" class="form-control" placeholder="No. pengeluaran / SPK..." value="<?= htmlspecialchars($search) ?>">
        </div>
    ...
    <div class="table-card">
      <div class="table-header">
        <h4><i class="bi bi-box-arrow-up"></i> Daftar Pengeluaran <span class="count-badge"><?= count($list) ?></span></h4>
      ...
      <div class="table-wrap">
        <table id="pengeluaranTable">
          <thead>
            <tr><th>No</th><th>Nomor Pengeluaran</th><th>SPK</th><th>Tanggal</th><th>Status</th><th>PIC</th><th>Aksi</th></tr>
          </thead>
          <tbody>
            <?php if (empty($list)): ?>
            <tr><td colspan="7" class="empty-state"><i class="bi bi-box-arrow-up"></i><span>Belum ada data. <a href="crud/add.php">Tambah sekarang</a></span></td></tr>
            <?php else: ?>
            <?php foreach ($list as $i => $row): ?>
            <tr>
              <td class="text-muted"><?= $i+1 ?></td>
              <td class="fw-mid"><?= htmlspecialchars($row['nomor_pengeluaran']) ?></td>
              <td class="text-muted"><?= htmlspecialchars($row['nomor_spk'] ?? '—') ?></td>
              ...
              <td>
                <div class="action-btns">
                  ...
                  <button type="button" class="btn-icon danger" onclick="confirmDelete(<?= $row['id'] ?>, '<?= htmlspecialchars($row['nomor_pengeluaran'], ENT_QUOTES) ?>')"><i class="bi bi-trash"></i></button>
                </div>
              </td>
            </tr>
            <?php endforeach; ?>
