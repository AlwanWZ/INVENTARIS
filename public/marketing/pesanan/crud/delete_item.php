...
$item = Pesanan::getItem($_GET['id'] ?? null);
if (!$item) {
    header('Location: ../index.php');
    exit;
}

$pesanan = Pesanan::find($item['pesanan_id']);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        // 2. Hapus item
        Pesanan::deleteItem($item['id']);

        $pdo->commit();
        header('Location: detail.php?id=' . $item['pesanan_id'] . '&item_deleted=1');
        exit;
    } catch (Exception $e) {
        $pdo->rollBack();
        ...
        <div style="padding: 2rem; text-align: center;">
          ...
          <div style="background: #f8f9fa; padding: 1.5rem; border-radius: 4px; margin-bottom: 2rem; text-align: left;">
            <div style="margin-bottom: 1rem;">
              <strong>Pesanan:</strong> <?= htmlspecialchars($pesanan['nomor_pesanan']) ?>
            </div>
            <div style="margin-bottom: 1rem;">
          ...
          <a href="detail.php?id=<?= $item['pesanan_id'] ?>" class="btn-outline" style="padding: 0.75rem 2rem; margin-left: 1rem;">
