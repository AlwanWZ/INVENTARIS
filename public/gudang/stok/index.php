...
// FIX: Hapus p.stok_ng dari query
$sql = "
    SELECT p.id, p.kode, p.nama, p.satuan, p.stok,
  ...
  <div class="content">
    ...
    <div class="stat-row">
      ...
      <div class="stat-pill">
        ...
        <span class="stat-pill-val ok"><?= number_format($totalFG) ?></span>
      </div>
      <div class="stat-pill">
        <span class="stat-pill-label">Stok Kritis</span>
        <span class="stat-pill-val <?= count($kritisItems) > 0 ? 'danger' : 'ok' ?>"><?= count($kritisItems) ?></span>
      </div>
    </div>
        ...
        <table id="stokTable">
          ...
          <tbody>
            ...
            <?php foreach ($listProduk as $i => $row):
              ...
              $pct  = $maxStok > 0 ? min(100, round($row['stok'] / $maxStok * 100)) : 0;
              $trCls= $cls === 'danger' ? 'row-kritis' : '';
            ?>
            <tr class="<?= $trCls ?>">
              ...
              <td class="col-right">
                <span class="stok-<?= $cls === 'ok' ? 'fg' : ($cls === 'warn' ? 'warn' : 'danger') ?>">
                  <?= number_format($row['stok']) ?>
                </span>
              </td>
              <td>
                <div class="stok-bar">
                  <div class="stok-bar-fill <?= $cls ?>" style="width:<?= $pct ?>%"></div>
                </div>
              </td>
              <td class="col-center">
                 <a href="../../gudang/laporan_persediaan/kartu_stok.php?produk_id=<?= $row['id'] ?>"
